...
if(sum(is.na(x$Prehen.Bin)>0)){x<-x[-which(is.na(x$Prehen.Bin)),]}
...
gelmanprior<-list(B=list(mu=rep(0,3), #set up priors for this model
                         V=gelman.prior(~as.factor(A)+zLength,
                                        data = x,  scale=1+pi^2/3)), 
                  R=list(V=1,fix=1),G=list(G1=list(V=1E-10,nu=-1)))

mod<-MCMCglmm(Prehen.Bin~as.factor(A)+zLength,
...
save(Final.disp,file="prehensile.Rdata")
