...
x<-x[which(x$Suborder=="Anomaluromorpha"),]
...
#if(sum(is.na(x$F)>0)){x<-x[-which(is.na(x$F)),]} #fossoriality removed from model, as there are no fossorial Anomaluromorpha
...
mod<-MCMCglmm(zLength~zLS+zTemp+as.factor(A), #can also remove the litter size variable if you'd like (remember not to subset above and to remove it in the model below)
...
save(Final.mod,file="tail-length-A.Rdata")
...
for(i in 1:100){ #loop through 100 trees
  ...
  mod<-MCMCglmm(zLength~zLS+zTemp+as.factor(A),
                random=~UphamTreeName.full, 
                ginverse=list(UphamTreeName.full=animalA), 
                prior = prior.PN, 
                verbose=FALSE, 
