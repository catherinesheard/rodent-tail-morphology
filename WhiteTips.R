...
if(sum(is.na(x$White.tips)>0)){x<-x[-which(is.na(x$White.tips)),]}
...
gelmanprior<-list(B=list(mu=rep(0,9), #set up priors for this model
                         V=gelman.prior(~as.factor(A)+as.factor(Q)+as.factor(Noc)+as.factor(SocialGrpSize)+zShade, 
                                        data = x,  scale=1+pi^2/3)), 
                  R=list(V=1,fix=1),G=list(G1=list(V=1E-10,nu=-1)))

mod<-MCMCglmm(as.factor(White.tips)~as.factor(A)+as.factor(Q)+as.factor(Noc)+as.factor(SocialGrpSize)+zShade,
...
save(Final.disp,file="white-tips.Rdata")
