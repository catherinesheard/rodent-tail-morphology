...
if(sum(is.na(x$Contrasting)>0)){x<-x[-which(is.na(x$Contrasting)),]}
...
#note that social group size is here categorical -- it can instead be scaled into a pseudo-continuous variable
...
gelmanprior<-list(B=list(mu=rep(0,8), #set up priors for this model
                         V=gelman.prior(~as.factor(A)+as.factor(Q)+zShade+as.factor(SocialGrpSize), 
...
mod<-MCMCglmm(as.factor(Contrasting)~as.factor(A)+as.factor(Q)+zShade+as.factor(SocialGrpSize),
...
save(Final.disp,file="contrasting.Rdata")
