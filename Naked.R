...
if(sum(is.na(x$Naked)>0)){x<-x[-which(is.na(x$Naked)),]}
...
gelmanprior<-list(B=list(mu=rep(0,2), #set up priors for this model
...
mod<-MCMCglmm(as.factor(Naked)~zTemp,
...
save(Final.disp,file="naked.Rdata")
