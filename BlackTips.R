...
if(sum(is.na(x$Black.tips)>0)){x<-x[-which(is.na(x$Black.tips)),]}
...
gelmanprior<-list(B=list(mu=rep(0,7), #set up priors for this model
                         V=gelman.prior(~as.factor(Di)+as.factor(SocialGrpSize)+zShade, 
...
mod<-MCMCglmm(as.factor(Black.tips)~as.factor(Di)+as.factor(SocialGrpSize)+zShade,
...
save(Final.disp,file="blacktips.Rdata")
