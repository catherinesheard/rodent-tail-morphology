...
if(sum(is.na(x$Fluffy)>0)){x<-x[-which(is.na(x$Fluffy)),]}
...
if(sum(is.na(x$SnekThreat)>0)){x<-x[-which(is.na(x$SnakeThreat)),]}
if(sum(is.na(x$Mass)>0)){x<-x[-which(is.na(Mass)),]}
...
gelmanprior<-list(B=list(mu=rep(0,9), #set up priors for this model
                         V=gelman.prior(~zTemp+as.factor(SocialGrpSize)+as.factor(Di)+zSnake+zMass, 
...
mod<-MCMCglmm(as.factor(Fluffy)~zTemp+as.factor(SocialGrpSize)+as.factor(Di)+zSnake+zMass,
...
save(Final.disp,file="fluffy.Rdata")
